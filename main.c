...
void main(void){
    ...
    while(1){
        ...
        if(key =! 'x'){
            num_to_disp = num_to_disp<< 8;
            num_to_disp = num_to_disp + key;
        }
        send_to_disp(num_to_disp);
...
char key_scanner(void){
    ...
    LATAbits.LA3 = 0;
    __delay_ms(SWEEP_FREQ);
    if      (PORTAbits.RA4 == 0) {__delay_ms(SWEEP_FREQ); return 15;}
    else if (PORTAbits.RA5 == 0) {__delay_ms(SWEEP_FREQ); return 0;}
    else if (PORTAbits.RA6 == 0) {__delay_ms(SWEEP_FREQ); return 14;}
